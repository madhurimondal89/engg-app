    ...
    const getFormulaInfo = () => {
        if (activeCalculator === 'cutting-speed') {
            return {
                ...
                description: 'Calculates cutting speed (V) based on workpiece or tool diameter (D) and spindle speed (N).'
            };
        } else if (activeCalculator === 'mrr') {
            return {
                name: 'Material Removal Rate (MRR)',
                formula: 'MRR = d × w × Vf',
                description: 'Rate of material removed based on depth of cut (d), width of cut (w), and feed rate (Vf).'
            };
        }
        return {
            name: activeCalculator.replace('-', ' ').replace(/\b\w/g, c => c.toUpperCase()),
            formula: 'Formula specific to manufacturing processes',
            description: 'Refer to manufacturing guidelines for detailed formulas on ' + activeCalculator.replace('-', ' ')
        };
    };
